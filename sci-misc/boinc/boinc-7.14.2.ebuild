...
inherit autotools desktop eapi7-ver gnome2-utils linux-info user wxwidgets
...
PATCHES=(
	...
	# fix sandboxing tools installation
	"${FILESDIR}"/0001-Install-switcher-tool-to-FHS-directory.patch
	"${FILESDIR}"/0002-Install-missing-tool-for-sandboxing.patch
	"${FILESDIR}"/0003-Add-missing-config.h-include-in-setprojectgrp.cpp.patch
)
...
src_prepare() {
	...
	# fix systemd unit installation path
	sed -i -e "s:usr/lib:lib:" client/scripts/Makefile.am || die
...
src_configure() {
	# TODO Xss is automagic on --enable-client. 
...
src_install() {
	...
	# Give X access to boinc for idle detection
	exeinto /etc/X11/xinit/xinitrc.d
	newexe "${FILESDIR}"/90-boincxss 90-boincxss
}
